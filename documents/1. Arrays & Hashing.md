...
We should find the three number of the array, sum of them equals to zero.
This problem is a two pointers problem.

first, I define two pointers, first one starts with index of 0, second one starts with index of [length - 1].
To solve this problem efficiently:
1. **Sort the array** to make it easier to avoid duplicates and use two pointers.
2. **Iterate** through the array, fixing one number (`i`), and use two pointers (`left` and `right`) to find the other two numbers such that their sum with `nums[i]` is zero.
3. For each fixed `i`, set `left = i + 1` and `right = nums.length - 1`.
4. Move the pointers:
    ...
    - If the sum is zero, record the triplet and skip duplicates for both `left` and `right`.
5. Skip duplicate values for `i` to avoid repeated triplets in the result.
6. Continue until all possible triplets are checked.
This approach reduces the time complexity to $O(n^2)$ and ensures all unique triplets are found.
...
This problem is very closed to the problem above, just add a `Math.abs` ,compare and set the min value to the `closest`.
...
The key of this problem is using a boolean flag to mark the next 1.
...
```
- **slow** moves one step at a time: `nums[slow]`
- **fast** moves two steps at a time: `nums[nums[fast]]`
- They will eventually meet inside the cycle (guaranteed by the pigeonhole principle)
...
return n1;
```
- Start `n1` from the beginning (0) and `n2` from where slow and fast met
- Move both pointers one step at a time
- They will meet at the entrance of the cycle, which is the duplicate number

## Example
For `nums = [1,3,4,2,2]`:
- Index:  `[0,1,2,3,4]`
- Values: `[1,3,4,2,2]`
- Linked list path: `0 → 1 → 3 → 2 → 4 → 2 → 4 → 2...` (cycle at 2)
- The duplicate is **2**
